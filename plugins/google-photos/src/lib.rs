...
/// (success, error, early return) releases the overlap guard.
...
impl GooglePhotosPlugin {
    ...
    fn sync_dir(&self) -> PathBuf {
        PathBuf::from(
            self.cfg
                .get_str("sync_dir")
                .unwrap_or("/tmp/picogallery-gdrive"),
        )
    }
    ...
    fn drive_root_args(&self) -> Vec<String> {
        match self.cfg.get_str("drive_folder_id") {
            Some(id) if !id.is_empty() => {
                ...
                if safe != id {
                    warn!(
                        "drive_folder_id contains invalid characters — stripped to '{}'",
                        safe
                    );
                }
                if safe.is_empty() {
    ...
    async fn run_rclone_authorize(&self) -> Result<()> {
        ...
        debug!(
            "rclone authorize exited {} ({} stdout / {} stderr bytes)",
            output.status,
            output.stdout.len(),
            output.stderr.len()
        );
    ...
    async fn sync_initial(&self) -> Result<()> {
        // Skip if a sync pass (foreground or background) is already running —
        // overlapping rclone runs would double bandwidth and disk churn.
        if self.sync_running.swap(true, Ordering::SeqCst) {
            debug!("Google Drive: sync already in progress — skipping initial sync.");
            return Ok(());
        }
        let _guard = SyncGuard(self.sync_running.clone());
    ...
    /// Spawn one rclone sync pass as a background task. On the current-thread
    /// runtime the task interleaves with the slideshow at await points (it is
    /// not parallel); the heavy lifting happens in the rclone child process.
    fn spawn_sync(&self) {
        // Same overlap guard as sync_initial — at most one rclone run at a time.
        if self.sync_running.swap(true, Ordering::SeqCst) {
            debug!("Google Drive: sync already in progress — skipping background sync.");
            return;
        }
        let guard = SyncGuard(self.sync_running.clone());
        ...
        tokio::spawn(async move {
            ...
            let mut args = vec![
                "--config".to_string(),
                conf.to_str().unwrap_or("").to_string(),
                "copy".to_string(),
                src.clone(),
...
impl PhotoPlugin for GooglePhotosPlugin {
    ...
    async fn list_photos(&self, limit: usize, offset: usize) -> Result<Vec<PhotoMeta>> {
        ...
        let mut paths = Self::list_local_images(&self.sync_dir()).await;
        ...
        // (the same offset never skips or duplicates between page calls) and so
        // the path-based id below maps to the same photo across calls, syncs,
        // and restarts.
        paths.sort();
